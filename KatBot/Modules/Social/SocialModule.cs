                ...
                switch (lovePower)
                {
                    case int i when i >= 1 && i <= 20:
                        await Context.Channel.SendColouredEmbedAsync($":crystal_ball: **({lovePower}%)** \n" +
                                                                     ...
                                                                     $"{person1} and {person2} don't seem to fit well together at all. :broken_heart:", Katarina.KatClr);
                        break;
                    case int i when i >= 21 && i <= 40:
                        await Context.Channel.SendColouredEmbedAsync($":crystal_ball: **({lovePower}%)** \n" +
                                                                     $"{printLoveBar(lovePower)}\n" +
                                                                     $"{person1} and {person2} are not likely to work out.", Katarina.KatClr);
                        break;
                    case int i when i >= 41 && i <= 60:
                        await Context.Channel.SendColouredEmbedAsync($":crystal_ball: **({lovePower}%)** \n" +
                                                                     $"{printLoveBar(lovePower)}\n" +
                                                                     $"{person1} and {person2} might have a chance together.", Katarina.KatClr);
                        break;
                    case int i when i >= 61 && i <= 80:
                        await Context.Channel.SendColouredEmbedAsync($":crystal_ball: **({lovePower}%)** \n" +
                                                                     $"{printLoveBar(lovePower)}\n" +
                                                                     $"{person1} and {person2} fit well for each other.", Katarina.KatClr);
                        break;
                    case int i when i >= 81 && i <= 100:
                        await Context.Channel.SendColouredEmbedAsync($":crystal_ball: **({lovePower}%)** \n" +
                                                                     $"{printLoveBar(lovePower)}\n" +
                                                                     $"{person1} and {person2} are perfect for each other! :heart:", Katarina.KatClr);
                        break;
