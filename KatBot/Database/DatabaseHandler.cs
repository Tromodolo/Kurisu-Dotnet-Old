...
namespace KatBot.Database
{
    public class DatabaseHandler
    {
        ...
        public async Task<BotData> fillBotDataAsync()
        {
            ...
            return new BotData
            {
                bottoken = EncryptionService.DecryptStringAES(data.bottoken, Katarina.encryptionpass),
                csecode = EncryptionService.DecryptStringAES(data.csecode, Katarina.encryptionpass),
                googleapikey = EncryptionService.DecryptStringAES(data.googleapikey, Katarina.encryptionpass),
                myanimelistname = EncryptionService.DecryptStringAES(data.myanimelistname, Katarina.encryptionpass),
                myanimelistpass = EncryptionService.DecryptStringAES(data.myanimelistpass, Katarina.encryptionpass),
                osuapikey = EncryptionService.DecryptStringAES(data.osuapikey, Katarina.encryptionpass)
            };
        }
        ...
        public async Task addRole(IRole role)
        {
            ...
            {
                serverid = EncryptionService.EncryptStringAES(role.Guild.Id.ToString(), Katarina.encryptionpass),
                roleid = EncryptionService.EncryptStringAES(role.Id.ToString(), Katarina.encryptionpass),
                rolename = EncryptionService.EncryptStringAES(role.Name, Katarina.encryptionpass)
            });
        ...
        public async Task removeRole(IRole role)
        {
            ...
            var res = await contextAsync.selfroles.FirstOrDefaultAsync(
                x => EncryptionService.DecryptStringAES(x.serverid, Katarina.encryptionpass) == role.Id.ToString() &&
                     EncryptionService.DecryptStringAES(x.rolename, Katarina.encryptionpass) == role.Name);
        ...
        public async Task<string> getServerRole(ulong serverid)
        {
            ...
            var roles = await contextAsync.selfroles
                .Where(x => EncryptionService.DecryptStringAES(x.serverid, Katarina.encryptionpass) ==
                            serverid.ToString())
                .Select(x => EncryptionService.DecryptStringAES(x.rolename, Katarina.encryptionpass)).ToListAsync();
        ...
        public async Task<bool> checkServerRole(IRole role)
        {
            ...
            if (await contextAsync.selfroles.CountAsync(
                    x => EncryptionService.DecryptStringAES(x.serverid, Katarina.encryptionpass) ==
                         role.Guild.Id.ToString() &&
                         EncryptionService.DecryptStringAES(x.roleid, Katarina.encryptionpass) == role.Id.ToString()) >
                0)
                return true;
            return false;
        }
