...
using static KurisuBot.Services.OsuModels;

namespace KurisuBot.Services
{
    public class OsuMethods
    {
        ...
        public static async Task<List<OsuUserBestScore>> GetUserBestAsync(string userId, int gamemode, int limit = 5)
        {
            var urlRequest =
                await GetAsync(
                    $"{RootDomain}{GetUserBestUrl}{ApiKeyParameter}{Kurisu.botData.osuapikey}{UserParameter}{userId}{ModeParameter}{gamemode}{LimitParameter}{limit}");
        ...
        public static async Task<List<OsuUserBestScore>> GetUserRecentAsync(string userId, int gamemode, int limit = 5)
        {
            var urlRequest =
                await GetAsync(
                    $"{RootDomain}{GetUserRecentUrl}{ApiKeyParameter}{Kurisu.botData.osuapikey}{UserParameter}{userId}{ModeParameter}{gamemode}{LimitParameter}{limit}");
            var maps = JsonConvert.DeserializeObject<List<OsuUserBestScore>>(urlRequest);
            return maps;
        }
        ...
        public static async Task<OsuBeatMap> GetBeatmapAsync(ulong beatmapId, int gamemode)
        {
            var urlRequest =
                await GetAsync(
                    $"{RootDomain}{GetBeatmapsUrl}{ApiKeyParameter}{Kurisu.botData.osuapikey}{BeatmapParameter}{beatmapId}");
        ...
        public static async Task<OsuUser> GetUserAsync(string username, int gamemode)
        {
            var urlRequest =
                await GetAsync(
                    $"{RootDomain}{GetUserUrl}{ApiKeyParameter}{Kurisu.botData.osuapikey}{UserParameter}{username}{ModeParameter}{gamemode}");
